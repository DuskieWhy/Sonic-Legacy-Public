...
class Alphabet extends FlxSpriteGroup
{
	...
	public var verticalSpacing:Float = 60;

	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1,mute:Bool = false)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 0, textSize,verticalSpacing);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacter = new AlphaCharacter(30 * loopNum, 0, textSize);
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 55 * yMulti, textSize,verticalSpacing);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class AlphaCharacter extends FlxSprite
{
	...
	private var verticalChange:Float = 60;

	public function new(x:Float, y:Float, textSize:Float,verticalChange:Float = 60)
	{
		super(x, y);
